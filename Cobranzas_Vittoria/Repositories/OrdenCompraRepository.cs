...
namespace Cobranzas_Vittoria.Repositories
{
    public class OrdenCompraRepository : RepositoryBase, IOrdenCompraRepository
    {
        ...
        {
            using var db = Open();
            return await db.QueryAsync<OrdenCompra>("compras.usp_OrdenCompra_List",
        ...
        {
            using var db = Open();
            using var multi = await db.QueryMultipleAsync("compras.usp_OrdenCompra_Get",
                new { IdOrdenCompra = idOrdenCompra }, commandType: CommandType.StoredProcedure);
        ...
        public async Task<(int IdOrdenCompra, decimal Total)> CrearAsync(OrdenCompraCreateDto dto)
        {
            ...
            var res = await db.QueryFirstAsync<dynamic>("compras.usp_OrdenCompra_CrearDesdeRequerimiento", p, commandType: CommandType.StoredProcedure);
        ...
        {
            using var db = Open();
            await db.ExecuteAsync("compras.usp_OrdenCompra_ActualizarEstado",
